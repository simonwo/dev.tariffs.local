<?php
    ...
    if ($phase == "blocking_period_create") {
        get_formvars_blocking_period_create();
    }

    function get_formvars_blocking_period_create() {
        global $conn;
        $errors = array();
        pre($_REQUEST);
        //quit();
        ...
        if (count($errors) > 0) {
            ...
            $url = "/quota_blocking_period_create.html?action=new&err=1&quota_order_number_id=" . $quota_order_number_id;
        } else {
            $url = "/quota_blocking_period_create2.html";
        }
        header("Location: " . $url);
    }
