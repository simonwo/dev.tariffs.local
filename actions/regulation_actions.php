<?php
	...
	if ($phase == "regulation_create_edit") {
		get_formvars_regulation_create_edit();
	}
	...
	function get_formvars_regulation_create_edit() {
		global $conn;
		$errors = array();
		pre($_REQUEST);
		...
		if (count($errors) > 0) {
			...
			$url = "/regulation_create_edit.php?action=new&err=1";
			#h1 ("fail again");
		} else {
			#h1 ("succeed");
			$url = "/regulations.php";
