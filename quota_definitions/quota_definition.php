...
class quota_definition
{
    ...
    public function get_parameters()
    {
        ...
        if (isset($_GET["err"])) {
            ...
            if (empty($error_handler->error_string)) {
                ...
                if (!$ret) {
                    h1("An error has occurred - no such quota definition.");
                    die();
                }
            } else {
                $this->populate_from_cookies();
    ...
    {
        global $conn;
        $this->quota_definition_sid = get_querystring("quota_definition_sid");
        $this->quota_order_number_id = get_querystring("quota_order_number_id");
        $this->quota_order_number_sid = get_querystring("quota_order_number_sid");
        $sql = "select quota_definition_sid, validity_start_date, validity_end_date, description, initial_volume,
        measurement_unit_code, measurement_unit_qualifier_code, maximum_precision, critical_state, critical_threshold
		from quota_definitions qsp where quota_definition_sid = $1";
        $stmt = "quota_get_definition_" . uniqid();
        pg_prepare($conn, $stmt, $sql);
        $result = pg_execute($conn, $stmt, array($this->quota_definition_sid));
        ...
        if ($result) {
            ...
            $this->initial_volume = $row[4];
            $this->measurement_unit_code = $row[5];
            $this->measurement_unit_qualifier_code = $row[6];
            $this->maximum_precision = $row[7];
            $this->critical_state = $row[8];
            $this->critical_threshold = $row[9];
