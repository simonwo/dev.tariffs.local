...
<div class="govuk-form-group <?=$error_handler->get_error("snapshot_date_start");?>">
	<fieldset class="govuk-fieldset" aria-describedby="snapshot_hint" role="group">
		<legend class="govuk-fieldset__legend govuk-fieldset__legend--xl">
			<h1 class="govuk-fieldset__heading" style="max-width:100%;">Enter dates for snapshot</h1>
        </legend>
...
if (($scope == "mfn") || ($scope == "1011")) {
    ...
    mc.duty_amount, mc.monetary_unit_code, mc.measurement_unit_code, mc.measurement_unit_qualifier_code, m.validity_start_date, m.validity_end_date
    from ml.measures_real_end_dates m
    left outer join measure_components mc
    on m.measure_sid = mc.measure_sid
    where m.validity_start_date <= '" . $snapshot_date_start . "'
    and (m.validity_end_date is null or m.validity_end_date >= '" . $snapshot_date_start . "')
    and measure_type_id in ('103', '105') " . $range_clause . 
    ...
    left outer join measure_components mc
    on m.measure_sid = mc.measure_sid
    where m.validity_start_date <= '" . $snapshot_date_start . "' and (m.validity_end_date is null or m.validity_end_date >= '" . $snapshot_date_start . "')
    and measure_type_id in ('142', '145') " . $range_clause . 
...
<p style="text-align:justify">The table below shows the entire commodity code tree and show the prevalent duties applicable
    for the jurisdiction or duty type selected. Rows where the description text is greyed out are those rows where
...
<table cellspacing="0" class="govuk-table" id="table">
    <tr class="govuk-table__row">
        ...
        <th class="govuk-table__header c small" style="width:5%">Suffix</th>
        <th class="govuk-table__header c small" style="width:5%">Indent</th>
        <th class="govuk-table__header c small" style="width:5%">End-line?</th>
        <th class="govuk-table__header c small" style="width:5%">Assigned</th>
        <th class="govuk-table__header small" style="width:30%">Description</th>
        <th class="govuk-table__header l small" style="width:11%">Measure type</th>
        <th class="govuk-table__header r small" style="width:11%">Duty</th>
        <th class="govuk-table__header r small" style="width:9%">Start</th>
        <th class="govuk-table__header r small" style="width:9%">End</th>
    </tr>
<?php
    } else {
        ob_start();
...
?>
    <tr class="govuk-table__row<?=$match_class?>">
        ...
        <td class="govuk-table__cell vsmall" style="<?=$padding_string?>"><?=$commodity->format_description()?>
<?php
...
?>
        ...
        <td class="govuk-table__cell vsmall l"><?=$commodity->measure_type_id?><?=$commodity->measure_type_desc?></td>
