...
class quota_blocking_period
{
    ...
    public $quota_blocking_period_sid = "";
    public $blocking_period_type = null;
    public $quota_definition_sid = null;
    public $description = "";
    public $blocking_start_date = null;
    public $blocking_end_date = null;
    ...
    function populate_from_cookies()
    {
        ...
        $this->blocking_start_date = get_cookie("blocking_start_date");
        $this->blocking_end_date = get_cookie("blocking_end_date");
    }
    ...
    function validate_form()
    {
        ...
        $this->blocking_period_type = get_formvar("blocking_period_type", "", True);
        $this->description = get_formvar("description", "", True);

        $this->blocking_start_date_day = get_formvar("blocking_start_date_day", "", True);
        $this->blocking_start_date_month = get_formvar("blocking_start_date_month", "", True);
        $this->blocking_start_date_year = get_formvar("blocking_start_date_year", "", True);
        $this->blocking_start_date_string = $this->blocking_start_date_day . "|" . $this->blocking_start_date_month . "|" . $this->blocking_start_date_year;
        setcookie("blocking_start_date_string", $this->blocking_start_date_string, time() + (86400 * 30), "/");

        $this->blocking_end_date_day = get_formvar("blocking_end_date_day", "", True);
        $this->blocking_end_date_month = get_formvar("blocking_end_date_month", "", True);
        $this->blocking_end_date_year = get_formvar("blocking_end_date_year", "", True);
        $this->blocking_end_date_string = $this->blocking_end_date_day . "|" . $this->blocking_end_date_month . "|" . $this->blocking_end_date_year;
        setcookie("blocking_end_date_string", $this->blocking_end_date_string, time() + (86400 * 30), "/");
        ...
        # Check on the blocking start date
        $valid_start_date = checkdate($this->blocking_start_date_month, $this->blocking_start_date_day, $this->blocking_start_date_year);
        if ($valid_start_date != 1) {
            array_push($errors, "blocking_start_date");
        }

        # Check on the blocking end date
        $valid_end_date = checkdate($this->blocking_end_date_month, $this->blocking_end_date_day, $this->blocking_end_date_year);
        if ($valid_end_date != 1) {
            array_push($errors, "blocking_end_date");
        }

        # Check on the blocking period type
        if (($this->blocking_period_type == "Unspecified") || ($this->blocking_period_type == "")) {
            array_push($errors, "blocking_period_type");
        }
    ...
    function get_next_sid()
    {
        ...
        $minimum = $application->minimum_sids["quota.blocking.periods"];
        $sql = "select max(quota_blocking_period_sid) as maximum from quota_blocking_periods qsp;";
    ...
    public function get_parameters()
    {
        ...
        elseif ($application->mode == "insert") {
            ...
            if (empty($error_handler->error_string)) {
                ...
                if (!$ret) {
                    h1("An error has occurred - no such blocking period.");
                    die();
                }
            } else {
                $this->populate_from_cookies();
    ...
    public function clear_cookies()
    {
        ...
        setcookie("blocking_start_date", "", time() + (86400 * 30), "/");
        setcookie("blocking_end_date", "", time() + (86400 * 30), "/");
    }
}
